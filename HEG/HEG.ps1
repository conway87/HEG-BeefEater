...
# UI
$TacticColourFG = 'White'
$TacticColourBG = 'Magenta'
$TacticDescColourFG = "White"
$TacticDescColourBG = "Green"
$TacticDetailsColour = "Green"
$TechniqueColour = "White"
$StatusColour = "Gray"
$LogoText_FG_Colour = "Black"
$LogoText_BG_Colour = "Red"
$LogoColour = "White"
...
Write-Host "          ██║  ██║   ██╔════╝   ██╔════╝    " -ForegroundColor $LogoColour -NoNewline; Write-Host "  " -NoNewline; Write-Host "The BeefEater Edition"-ForegroundColor $LogoText_FG_Colour -BackgroundColor $LogoText_BG_Colour
Write-Host "          ███████║   █████╗     ██║  ███╗   " -ForegroundColor $LogoColour -NoNewline; Write-Host "  " -NoNewline; Write-Host "Not For Casuals"  -ForegroundColor $LogoText_FG_Colour -BackgroundColor $LogoText_BG_Colour
Write-Host "          ██╔══██║   ██╔══╝     ██║   ██║   " -ForegroundColor $LogoColour -NoNewline; Write-Host "  " -NoNewline; Write-Host "Time To Get Your Grep On" -ForegroundColor $LogoText_FG_Colour -BackgroundColor $LogoText_BG_Colour
Write-Host "          ██║  ██║██╗███████╗██╗╚██████╔╝██╗" -ForegroundColor $LogoColour
Write-Host "          ╚═╝  ╚═╝╚═╝╚══════╝╚═╝ ╚═════╝ ╚═╝" -ForegroundColor $LogoColour
...
# Keep track of the current sections to print headers
$currentSection = ""
$currentTacticDetails = ""
...
foreach ($item in $items) {
    if ($currentSection -ne $item.Tactic) {
        ...
        Write-Host "$tacticDescription" -ForegroundColor $TacticDescColourFG -BackgroundColor $TacticDescColourBG 
    }


    # Print Technique and Technique Details when testing moves to next Technique
    if ($currentTacticDetails -ne $item.TacticDetails) {
        $currentTacticDetails = $item.TacticDetails
        Write-Host "`n          $currentTacticDetails" -ForegroundColor $TacticDetailsColour
    }
    ...
    # Print the description to screen along with appending it into the events_timeline.txt file
    Write-Host "                    $description" -ForegroundColor $TechniqueColour
    "$(Get-Date -Format G) - $currentTacticDetails - $description - $EventID" | Out-File -FilePath "$logfile" -Append
...
if ($item.Script -and $item.Script.Trim() -ne "") {
    ...
    # Write Timestamp and 'Complete' after each iteration and also append it into the events_timeline.txt file
    "$(Get-Date -Format G) - Complete `n" | Out-File -FilePath "$logfile" -Append
}
